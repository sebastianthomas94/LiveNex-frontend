...
export default function Login() {
  ...
  const handleSignin = (e)=>{
    ...
    if(!email || !password)
    {
        toast.dark("Enter the credentials to sign");
        return;
    }
    signin({ email, password}).unwrap()
        ...
        .catch((e)=>{
            ...
            if(e.status === 401) toast.error("Invalid password");
            if(e.status === 500) toast.error("Internal server error");
        });
  };
    ...
    <div>
        ...
        <div className="lg:flex w-1/2 hidden bg-gray-500 bg-no-repeat bg-cover relative items-center" /* style="background-image: url(https://images.unsplash.com/photo-1577495508048-b635879837f1?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=675&q=80);" */>
            ...
            <div className="w-full px-24 z-10">
                <h1 className="text-5xl font-bold text-left tracking-wide">Unlock the World's Stage: </h1>
                <p className="text-3xl my-4">LiveNex - Your Gateway to Seamless Live Streaming</p>
            </div>
        ...
        <div className="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-0 z-0" /* style="background-color: #161616;" */>
          ...
          <div className="mt-4">
              ...
              >
                ...
                <form onSubmit={handleSignin} className="sm:w-2/3 w-full px-4 lg:px-0 mx-auto">
                    <div className="pb-2 pt-4">
                        <input type="email" name="email" id="email" placeholder="Email" className="block w-full p-4 text-lg rounded-sm bg-white"
                        value={email}
                        onChange={(e)=>setEmail(e.target.value)}
                        />
                    </div>
                    <div className="pb-2 pt-4">
                        <input className="block w-full p-4 text-lg rounded-sm bg-white" type="password" name="password" id="password" placeholder="Password"
                        value={password}
                        onChange={(e) => setPassword(e.target.value)}
                        />
                    </div>
                    <div className="text-right text-gray-400 hover:underline hover:text-gray-100">
                        <a href="q">Forgot your password?</a>
                    </div>
